...
<img src="./.readme/badge/version.svg" alt="version 2.6.18">
...
The widget form is deliberately small - pick a **provider**, one of the **admin-managed tokens** and a **model**; everything sensitive (the key, a custom endpoint, tuning parameters) lives on the token under **Administration → AI General**. A saved key renders only as a mask of its length - the value never travels to the browser, and the reveal eye fetches it on an explicit click. PRO-only fields show inline (greyed with a padlock in the FREE edition).

In PRO, switching providers and back while editing restores the model you selected for each provider, including when switching through a custom endpoint. The same applies when changing tokens between providers in global configuration. A saved model that is still supported stays selected when you reopen the form; an unavailable model falls back to the provider's default.
...
| Follow-latest scrolling with a return-to-latest control       |   ✅   |   ✅   |
